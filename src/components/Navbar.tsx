...
export default function Navbar() {
  ...
  <img src="/logo.png" alt="Core Academy" className="h-10 w-auto" />
</Link>
          ...
          <div className="hidden md:flex items-center gap-16 lg:gap-20">
            {navLinks.map((link) => (
              <a 
                key={link.name} 
                href={link.href}
                className="text-xs font-black uppercase tracking-[0.2em] text-white/50 hover:text-brand-cyan transition-all hover:scale-110 active:scale-95 italic"
              >
                {link.name}
              </a>
            ))}
      ...
      {/* Mobile Nav Overlay - Now outside the transforming container */}
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div
            initial={{ opacity: 0 }}
            ...
            className="fixed inset-0 bg-brand-navy/60 backdrop-blur-[60px] z-[9999] md:hidden flex flex-col p-8 pt-32 overflow-y-auto"
          >
            {/* Close Button Inside Menu */}
            <button 
              className="absolute top-6 right-6 p-4 text-white rounded-full bg-white/10 border border-white/20"
              onClick={() => setIsMenuOpen(false)}
            >
              <X className="w-8 h-8 text-brand-cyan" />
            </button>
            ...
            <div className="mt-auto space-y-6">
              ...
              >
                <a href="https://www.instagram.com/core_academy_edu" target="_blank" className="text-white/40 hover:text-white transition-colors">
